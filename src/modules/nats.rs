...
/// The default NATS symbol. Keep the trailing space in the theme value so a
/// custom theme can replace the complete marker cleanly.
...
impl<S: NatsScheme> Module for Nats<S> {
    fn append_segments(&mut self, powerline: &mut Powerline) {
        ...
        let Lookup::Ready(name) = Cached::new(NatsLookup { binary }).load() else {
            // A failed or not-yet-complete context lookup is omitted.
            return;
        };
