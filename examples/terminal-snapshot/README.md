...
- `vhs-fixes.patch`: two fixes for VHS v0.12.0: render with a live context so
  screenshots are written (charmbracelet/vhs#787), and pass ttyd a working
  directory so its Windows build can spawn the shell (tsl0922/ttyd#1413).
  Drop it once releases include both.
